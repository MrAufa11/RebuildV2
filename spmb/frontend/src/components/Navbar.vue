<template>
  <nav id="navbar" :class="['w-full z-50 transition-all duration-300', (isScrolled || !isHome) ? 'fixed top-0 scrolled' : 'absolute top-0 py-6 border-b border-white/10']">
    <div class="max-w-7xl mx-auto px-6 flex justify-between items-center">
      ...
      <div class="flex items-center gap-3 transition-colors duration-300" :class="(isScrolled || !isHome) ? 'text-dark' : 'text-white'">
        <div class="w-10 h-10 bg-brand-orange rounded-xl flex items-center justify-center text-white text-xl shadow-lg shadow-orange-500/30 backdrop-blur-sm">
          <i class="fas fa-book-open"></i>
        </div>
        <span class="font-bold text-xl tracking-wide font-serif drop-shadow-md">Al-Mawahib<span class="text-brand-orange">.sch</span></span>
      </div>
