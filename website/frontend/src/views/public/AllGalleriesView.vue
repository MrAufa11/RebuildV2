        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="flex flex-wrap justify-center gap-3 mb-12" data-aos="fade-up" data-aos-delay="100">
                <button 
                    ...
                    :class="activeCategory === 'all' ? 'bg-brand-orange text-white border-brand-orange shadow-lg shadow-brand-orange/30' : 'bg-white text-gray-600 border-gray-200 hover:border-brand-orange hover:text-brand-orange'"
                >
                    Semua
                </button>
                <button 
                    ...
                    class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 border"
                    :class="activeCategory === cat.id ? 'bg-brand-orange text-white border-brand-orange shadow-lg shadow-brand-orange/30' : 'bg-white text-gray-600 border-gray-200 hover:border-brand-orange hover:text-brand-orange'"
                >
                    {{ cat.name }}
                </button>
            </div>
                ...
                >
                    ...
                    <div class="absolute bottom-0 left-0 p-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                        <span class="inline-block px-3 py-1 bg-brand-orange/90 text-white text-xs font-bold rounded-full mb-2 backdrop-blur-sm">
                            {{ item.website_category ? item.website_category.name : 'Kegiatan' }}
                        </span>
            ...
            <div v-if="!loading && filteredGalleries.length === 0" class="text-center py-20 bg-white rounded-3xl shadow-sm">
                <i class="far fa-images text-6xl text-slate-200 mb-4"></i>
                <p class="text-slate-500 font-medium">Belum ada foto galeri untuk kategori ini.</p>
            </div>
        </div>
...
const filteredGalleries = computed(() => {
    ...
    return galleries.value.filter(item => item.website_category?.id === activeCategory.value);
});
