...
const hasPermission = (menuLabel) => {
    ...
    // If user is superadmin (optional check, assuming role name 'Super Admin' or similar has all access, 
    // but here we stick to strict permission check as requested)
    
    // Check if user has *.read permission for this menu
    const menuSlug = slugify(menuLabel);
    const requiredPermission = `${menuSlug}.read`;
    
    // Check in user permissions (assuming user object has permissions array of strings or objects)
    const userPermissions = currentUser.value.role ? currentUser.value.role.permissions : [];
    
    // Normalize permissions to strings if they are objects
...
const filteredMenus = computed(() => {
    return menus.value; // Backend already filtered by permission/app
});

const fetchCurrentUser = async () => {
    try {
        const response = await api.get('/auth/me', { params: { app_type: 'website' } });
