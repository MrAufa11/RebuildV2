<template>
    <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-all duration-300">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="font-bold text-gray-900 dark:text-white">{{ isEditing ? 'Edit Menu' : 'Create New Menu' }}</h3>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="p-6">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    >
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">The route path for this menu item.</p>
                </div>

                <!-- Icon -->
                <div>
                    <label for="icon" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Icon Class (FontAwesome)</label>
                    <div class="flex items-center gap-2">
                        <div class="w-10 h-10 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-lg text-gray-600 dark:text-gray-300">
                             <i :class="[form.icon || 'fas fa-question']"></i>
                        </div>
                        <input 
                            v-model="form.icon" 
                            type="text" 
                ...
                <!-- Parent Menu -->
                <div>
                    <label for="parent_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Parent Menu (Optional)</label>
                    <select 
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-700 mt-6">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-brand-600 border border-transparent rounded-lg shadow-sm hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-all shadow-brand-500/30 cursor-pointer">
                    <i class="fas fa-save mr-2"></i> {{ isEditing ? 'Update Menu' : 'Create Menu' }}
                </button>
            </div>
        </form>
    </div>
...
const fetchParentOptions = async () => {
    try {
        ...
        // Filter out menus that are children themselves (assuming only 1 level deep nesting for simplicity, or allow nesting but prevent self-referencing)
        // For simplicity, let's just show all top-level menus
        // Wait, the API returns tree structure? Yes, standard getAll returns tree.
        // We need flat list for select, but only valid parents.
        // Actually, if getAll returns tree, we can just use the top level items as potential parents.
        parentOptions.value = response.data.filter(m => !m.parent_id);
    } catch (error) {
...
const fetchMenu = async (id) => {
    try {
        ...
        Alert.error('Error', 'Failed to load menu data');
...
const handleSubmit = async () => {
    try {
        if (isEditing.value) {
            await api.put(`/menus/${route.params.id}`, form.value);
        ...
        Alert.error('Error', error.response?.data?.message || 'Failed to save menu');
    }
};
...
onMounted(async () => {
    ...
    if (isEditing.value) {
        await fetchMenu(route.params.id);
    }
});
</script>
