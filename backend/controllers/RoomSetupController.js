...
const RoomSetupController = {
    async getAll(req, res) {
        try {
            const records = await spmbDb.query('SELECT * FROM room_setup ORDER BY id DESC', { type: QueryTypes.SELECT });
    ...
    async getById(req, res) {
        try {
            const [record] = await spmbDb.query('SELECT * FROM room_setup WHERE id = ?', {
    ...
    async create(req, res) {
        try {
            ...
            console.error('Error in create for room_setup:', error);
