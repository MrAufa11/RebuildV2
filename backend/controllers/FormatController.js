...
const FormatController = {
    async getAll(req, res) {
        try {
            const records = await spmbDb.query('SELECT * FROM format ORDER BY id DESC', { type: QueryTypes.SELECT });
    ...
    async getById(req, res) {
        try {
            const [record] = await spmbDb.query('SELECT * FROM format WHERE id = ?', {
    ...
    async create(req, res) {
        try {
            ...
            await spmbDb.query(`INSERT INTO format (${keys.join(', ')}) VALUES (${placeholders})`, {
                replacements: values, type: QueryTypes.INSERT
            });
            return res.status(201).json({ message: 'Created successfully' });
        } catch (error) {
            console.error('Error in create for format:', error);
    ...
    async update(req, res) {
        try {
            ...
            await spmbDb.query(`UPDATE format SET ${setString} WHERE id = ?`, {
    ...
    async delete(req, res) {
        try {
            await spmbDb.query('DELETE FROM format WHERE id = ?', {
