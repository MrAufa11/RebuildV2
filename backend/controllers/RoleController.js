...
const syncPermissions = async (roleId, permissions) => {
    ...
    if (!permissions || !Array.isArray(permissions)) {
        console.log('Permissions invalid or empty');
        return;
    }

    const menus = await Menu.findAll();
    console.log(`Fetched ${menus.length} menus from DB`);
    ...
    console.log('Permission Map keys:', Object.keys(permMap));

    const permissionData = [];
    menus.forEach(menu => {
        const slug = slugify(menu.label);
        // console.log(`Menu: ${menu.label} -> Slug: ${slug}`);
        if (permMap[slug]) {
            permissionData.push({
                role_id: roleId,
                menu_id: menu.id,
    ...
    console.log(`Prepared ${permissionData.length} permission records for insertion`);
