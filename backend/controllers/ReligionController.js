...
const ReligionController = {
    async getAll(req, res) {
        try {
            const records = await spmbDb.query('SELECT * FROM religion ORDER BY id DESC', { type: QueryTypes.SELECT });
    ...
    async getById(req, res) {
        try {
            const [record] = await spmbDb.query('SELECT * FROM religion WHERE id = ?', {
    ...
    async create(req, res) {
        try {
            ...
            await spmbDb.query(`INSERT INTO religion (${keys.join(', ')}) VALUES (${placeholders})`, {
                replacements: values, type: QueryTypes.INSERT
            });
            return res.status(201).json({ message: 'Created successfully' });
        } catch (error) {
            console.error('Error in create for religion:', error);
    ...
    async update(req, res) {
        try {
            ...
            await spmbDb.query(`UPDATE religion SET ${setString} WHERE id = ?`, {
    ...
    async delete(req, res) {
        try {
            await spmbDb.query('DELETE FROM religion WHERE id = ?', {
