...
const IncomeController = {
    async getAll(req, res) {
        try {
            const records = await spmbDb.query('SELECT * FROM income ORDER BY id DESC', { type: QueryTypes.SELECT });
    ...
    async getById(req, res) {
        try {
            const [record] = await spmbDb.query('SELECT * FROM income WHERE id = ?', {
    ...
    async create(req, res) {
        try {
            ...
            await spmbDb.query(`INSERT INTO income (${keys.join(', ')}) VALUES (${placeholders})`, {
                replacements: values, type: QueryTypes.INSERT
            });
            return res.status(201).json({ message: 'Created successfully' });
        } catch (error) {
            console.error('Error in create for income:', error);
    ...
    async update(req, res) {
        try {
            ...
            await spmbDb.query(`UPDATE income SET ${setString} WHERE id = ?`, {
    ...
    async delete(req, res) {
        try {
            await spmbDb.query('DELETE FROM income WHERE id = ?', {
