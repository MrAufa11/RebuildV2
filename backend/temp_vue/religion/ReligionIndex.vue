<template>
    <div class="max-w mx-auto space-y-8">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Religion</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Manajemen data referensi Religion.</p>
            </div>
            
            <router-link :to="{ name: 'religion-create' }" class="px-4 py-2 text-sm font-medium text-white bg-brand-600 border border-transparent rounded-lg shadow-sm hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-all shadow-brand-500/30">
        ...
        <div class="bg-white dark:bg-darkcard rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="p-5 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-end gap-4">
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" v-model="searchQuery" placeholder="Cari data Religion..." class="pl-9 pr-4 py-2 w-full border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-brand-500 outline-none">
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Religion Name</th>
                    ...
                    <tbody class="bg-white dark:bg-darkcard divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="(record, index) in filteredRecords" :key="record.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ record.religion_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <router-link :to="{ name: 'religion-edit', params: { id: record.id } }" class="text-brand-600 hover:text-brand-900 p-1 rounded-md hover:bg-brand-50 dark:hover:bg-brand-900/20"><i class="fas fa-edit"></i></router-link>
...
const filteredRecords = computed(() => {
    ...
    return records.value.filter(record => {
        return String(record.religion_name).toLowerCase().includes(q);
    });
});
