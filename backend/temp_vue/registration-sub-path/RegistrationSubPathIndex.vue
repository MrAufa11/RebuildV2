<template>
    <div class="max-w mx-auto space-y-8">
        <div class="flex justify-between items-center">
            ...
            <router-link :to="{ name: 'registration-sub-path-create' }" class="px-4 py-2 text-sm font-medium text-white bg-brand-600 border border-transparent rounded-lg shadow-sm hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-all shadow-brand-500/30">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Registration Path Id</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Sub Path Name</th>
                    ...
                    <tbody class="bg-white dark:bg-darkcard divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="(record, index) in filteredRecords" :key="record.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ record.registration_path_id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ record.sub_path_name }}</td>
...
const filteredRecords = computed(() => {
    ...
    return records.value.filter(record => {
        return String(record.registration_path_id).toLowerCase().includes(q) || String(record.sub_path_name).toLowerCase().includes(q);
    });
});
